...
export async function GET() {
  try {
    try { await sql`ALTER TABLE events ALTER COLUMN id TYPE VARCHAR(255);`; } catch (e) {}
    try { await sql`ALTER TABLE event_gallery_images ALTER COLUMN event_id TYPE VARCHAR(255);`; } catch (e) {}
    ...
    for (const act of ACTIVITIES) {
      ...
      if (res && res.length > 0) {
        logs.push(`Updated existing row for ${act.slug} (Matched ID: ${res[0].id})`);
        targetId = res[0].id;
      } else {
    ...
    return NextResponse.json({ success: true, message: "All 7 activities synchronized cleanly directly inside production database!", logs });
  } catch (error: any) {
