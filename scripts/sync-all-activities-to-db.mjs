...
const ACTIVITIES = [
  {
    ...
    time: "10:00 AM",
    location: "Cuddalore",
    cover: "/images/activities images/activity 1/img 1.jpg",
  ...
  },
  {
    ...
    cover: "/images/activities images/activity 2/img 1.jpg",
  ...
  },
  {
    ...
    time: "10:00 AM",
    location: "Cuddalore",
    cover: "/images/activities images/activity 3/img 1.jpg",
  ...
  },
  {
    ...
    time: "10:00 AM",
    location: "Cuddalore",
    cover: "/images/activities images/activity 4/img 1.jpg",
  ...
  },
  {
    ...
    time: "10:00 AM",
    location: "Cuddalore",
    cover: "/images/activities images/activity 5/img 1.jpg",
  ...
  },
  {
    ...
    time: "10:00 AM",
    location: "Cuddalore",
    cover: "/images/activities images/activity 6/img 1.jpg",
  ...
  },
  {
    ...
    time: "10:00 AM",
    location: "Cuddalore",
    cover: "/images/activities images/activity 7/img 1.jpg",
...
async function syncAll() {
  console.log("Starting full database sync for all 7 activities (UUIDs, slugs, numeric IDs)...");

  // Make sure gallery and videos tables accept text IDs
  try { await sql.query(`ALTER TABLE event_gallery_images ALTER COLUMN event_id TYPE VARCHAR(255) USING event_id::text;`); } catch (e) {}
  try { await sql.query(`ALTER TABLE event_videos ALTER COLUMN event_id TYPE VARCHAR(255) USING event_id::text;`); } catch (e) {}

  for (const act of ACTIVITIES) {
    const fullDesc = `${act.p1}\n\n${act.p2}\n\n${act.p3}`;

    // 1. Update existing row by uuid, slug, numericId, or legacyId
    const res = await sql`
      UPDATE events
      SET slug = ${act.slug},
          title = ${act.title},
    ...
    if (res && res.length > 0) {
      console.log(`Updated existing row for ${act.slug} (Matched ID: ${res[0].id})`);
      targetId = res[0].id;
    } else {
      console.log(`Row not found for ${act.slug}. Inserting new row with ID ${act.uuid}...`);
      await sql`
        INSERT INTO events (
    ...
    // 2. Sync gallery photos for ALL possible IDs (targetId, act.slug, act.numericId, act.uuid)
    const allIds = Array.from(new Set([String(targetId), act.slug, act.numericId, act.uuid]));
    ...
    console.log(`Synchronized ${act.gallery.length} gallery images across IDs [${allIds.join(', ')}] for ${act.slug}.`);
  }

  console.log("All 7 activities fully synchronized in Postgres!");
}
