...
async function syncAll() {
  console.log("Starting full database sync with exact HTTPS Cloudinary URLs across all 7 activities...");
  ...
  for (const act of ACTIVITIES) {
    ...
    if (res && res.length > 0) {
      console.log(`Updated existing row for ${act.slug} (Matched ID: ${res[0].id}) with Cloudinary cover: ${act.cover}`);
      targetId = res[0].id;
    } else {
    ...
    console.log(`Synchronized ${act.gallery.length} Cloudinary gallery images across IDs [${allIds.join(', ')}] for ${act.slug}.`);
  }

  console.log("All 7 activities fully synchronized with Cloudinary URLs in Postgres!");
}
